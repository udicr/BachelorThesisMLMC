 ...
 \begin{Definition}(Seiten- und Zellenbasis) 
	\begin{enumerate}[label=(\alph*)]
		\item $ \{ \psi_i \}_{i=1}^{N} $ heißt Seitenbasis und ist definiert durch
			...
			\end{align*} 
		\item $ \{ \mu_i \}_{m=1}^{|\mathcal{T}|} $ heißt Zellenbasis und ist gegeben durch
			\begin{align*}
				\forall m \in \{1, \dots , |\mathcal{T}| \} : \mu_m \coloneqq  \mathds{1}_{K_m}.
			\end{align*}
	\end{enumerate}
\end{Definition} 
...
\begin{Definition}(Ansatzräume)
	\begin{enumerate}[label=(\alph*)]
		...
		\item $ Q_h \coloneqq \spann \{ \mu_1, \dots, \mu_{|\mathcal{T}|} \} $ (Zellenansatzraum/ Raum für $\phi $ und $ u_h $)
	\end{enumerate}
\end{Definition}
