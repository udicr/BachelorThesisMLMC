...
%\end{align}
%Für jede feste Zelle $ K $ ergibt sich durch Einsetzen der Zellenbasis $ \{\mu_i\}_{i=1}^N \subset \mathcal{Q}_h $  eingeschränkt mit $ \supp(\mu_i) \subseteq \overline{K} $ für alle $\ i \in \{1, \dots , N\}$ : \\
%\begin{align*} 
%\left(\partial_t \rho_h, \mu_i  \right)_{K}  &= \bigg( \left(\Upsilon(\rho_h), \nabla\mu_i\right)_{K} - \sum_{\substack{F \in \mathcal{F}_{K} \\ F \not\subseteq \Gamma_{\text{in}}}} \left(\Upsilon^*(\rho_h) \cdot n^{K}, \mu_i \right)_{F} - \left(\rho_{\text{in}} q \cdot n^{K}, \mu_i \right)_{\partial K \cap \Gamma_{\text{in}}} \bigg) \\
%\end{align*}
...
%\underbrace{\left(\partial_t \rho_h, \mu_i  \right)_{K}}_{(\ref{DarstellungvorLGS}.1)} = 
%\underbrace{ \left(\Upsilon(\rho_h), \nabla\mu_i\right)_{K} -  \sum_{F \in \mathcal{F}_{K} , F \not \subseteq \Gamma_{\text{in}}}\left(\Upsilon^{\star}(\rho_h)\cdot n^{K},\mu_i\big)_F \right)}_{(\ref{DarstellungvorLGS}.2)} \underbrace{- \left(\rho_{\text{in}}q\cdot n^{K},\mu_i \right)_{\partial K \cap \Gamma_{\text{in}}}}_{(\ref{DarstellungvorLGS}.3)}
%\end{align}
%
