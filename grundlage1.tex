...
\begin{Definition}(Einige Operatoren)
	\begin{enumerate}[label=(\alph*)]
		\item Für $F \colon \R^d \to \R^d$ ist die Divergenz von F definiert durch
			\begin{align*}
				\dive  \colon \begin{cases}
				\R^d  \to \R \\
				F \mapsto \dive F= \nabla \cdot  F \coloneqq \sum_{i=1}^{d} \frac{\partial F_i}{ \partial x_i}
				\end{cases}
			\end{align*}
		\item Für $f \colon \R^d \to \R$ und $\alpha = (\alpha_1,\dots,\alpha_d) \in \N^d$ ist die partielle Ableitung von $f$ nach dem sogenannten Multiindex $\alpha$ definiert durch
			\begin{align*}
...
\begin{Satz} (Gaußscher Integralsatz für Lipschitz-Gebiete) \newline
 	Sei $\mathcal{D}  \subset \R^d$ ein beschränktes Lipschitz-Gebiet und sei $\nu$ der äußere Einheitsnormalenvektor an $\partial \mathcal{D}$ mit Komponenten $ \nu_i $. Dann gilt:
 		...
 		\end{align*}
 		wobei $F \colon \mathcal{D} \to \R^d$ ein Vektorfeld ist. Die Komponentenfunktionen von $F = (f_1,\dots,f_d)$ sollen dann $f_i \in C^1(\overline{\mathcal{D}})$ für $i=1,\dots,d$ erfüllen.
\end{Satz}

\begin{Folgerung}(mehrdimensionale partielle Integration) \\
	\label{n_pI}
	Sei $ f \in C^1(\R^d, \R)$ und $ F \colon \mathcal{D} \to \R^d $ ein stetig partiell differenzierbares Vektorfeld.
	Dann gilt:
...
\begin{Definition}(Sobolevräume)\\
	Sei $\mathcal{D} \subseteq \R^d $ offen, $ k \in \N $ und $ 1 \leq p \leq \infty $.  Weiter sei $ L \colon C^1(\mathcal{D},\R^m) \to L^{\infty}(\mathcal{D},\R^k) $ ein linearer Differentialoperator erster Ordnung und $ L^{\star} \colon C^1(\mathcal{D},\R^k) \to L^{\infty}(\mathcal{D},\R^m) $ der zugehörige adjungierte Operator. Es gelte also
	\[
	...
	\begin{enumerate}[label=(\alph*)]
		...
		$ W_0^{k,p}(\mathcal{D}) = \{ f \in W^{k,p}(\mathcal{D}) \colon f|_{\partial \mathcal{D}} = 0 \}$
		...
		\item $ H(L,\mathcal{D}) \coloneqq \{ f \in L^2(\mathcal{D},\R^m) \colon \exists v \in L^2(\mathcal{D},R^k) \text{ mit } \int_{\mathcal{D}} v \cdot \phi \dx = \int_{\mathcal{D}} f \cdot L^{\star}\phi \dx \ \forall \phi \in C^1_c(\mathcal{D},\R^k) \}$
	\end{enumerate}
\end{Definition}
